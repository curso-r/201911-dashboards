...
```{r, fig.height=4, fig.width=7}
pkmn %>%
  filter(tipo_1 == "fogo", !is.na(id_geracao)) %>% 
  mutate(
    geracao = paste("Geração ", id_geracao)
  ) %>%
  ggplot(aes(x = geracao)) + 
  geom_bar(fill = cores["fogo"]) + 
  labs(x = "", y = "Número de pokémon") +
  theme_minimal()
...
2. Acrescente mais uma aba à segunda seção, replicando os gráficos para algum outro tipo.
